...
public class Gaben extends GameObject  {
	...
	public Gaben(int x, int y, ID id, Handler handler, Game game, SpriteSheet Sprites) {
		super(x, y, id, Sprites, false);
		this.handler = handler;
		this.game = game;
		
		player_image_down[0] = Sprites.grabImage(1, 9, 32, 32);
		player_image_down[1] = Sprites.grabImage(2, 9, 32, 32);
		player_image_down[2] = Sprites.grabImage(3, 9, 32, 32);
		
		player_image_up[0] = Sprites.grabImage(1, 12, 32, 32);
		player_image_up[1] = Sprites.grabImage(2, 12, 32, 32);
		player_image_up[2] = Sprites.grabImage(3, 12, 32, 32);
		
		player_image_left[0] = Sprites.grabImage(1, 10, 32, 32);
		player_image_left[1] = Sprites.grabImage(2, 10, 32, 32);
		player_image_left[2] = Sprites.grabImage(3, 10, 32, 32);
		
		player_image_right[0] = Sprites.grabImage(1, 11, 32, 32);
		player_image_right[1] = Sprites.grabImage(2, 11, 32, 32);
		player_image_right[2] = Sprites.grabImage(3, 11, 32, 32);
	...
	private void collision() {
		for(int i = 0; i < handler.object.size(); i++) {
			...
			if(tempObject.getId() == ID.ScoreMedium) {
				if(getBounds().intersects(tempObject.getBounds())) {
					game.score += 25;
					handler.removeObject(tempObject);
			...
			if(tempObject.getId() == ID.ScoreSmall) {
				if(getBounds().intersects(tempObject.getBounds())) {
					game.score += 5;
					handler.removeObject(tempObject);
			...
			if(tempObject.getId() == ID.ScoreLarge) {
				if(getBounds().intersects(tempObject.getBounds())) {
					game.score += 100;
					handler.removeObject(tempObject);
	...
	public Rectangle getBounds() {
		return new Rectangle(x+6, y+4, 20, 24);
	}
